...
export default function modeControllerExtension(pi: ExtensionAPI): void {
  ...
  function updateModeIndicator(ctx: ExtensionContext): void {
    ...
    // Simple box showing current mode - positioned below editor
    const info = MODE_INFO[state.currentMode];
    const box = ` ${info.icon} ${info.label} `;
    const styled = ctx.ui.theme.fg(info.color, box);
    ctx.ui.setWidget("mode-controller", [styled], { placement: "belowEditor" });
  }
